...
import { apiUrl } from '@/app/lib/api'; // adjust path
...
export default function MemberDashboard() {
  ...
  const loadQR = async () => {
    ...
    try {
        const res = await fetch(apiUrl('/api/qr/me'), {
        headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        if (!data.success) {
            ...
            {!qrCode ? (
                ...
                <div className="flex gap-4 justify-center">
                    ...
                    onClick={() => {
                        ...
                        link.href = qrCode;
                        link.download = `${(member?.name || 'member').replace(/\s+/g, '_')}_QR.png`;
                        link.click();
                    }}
                    ...
                    <button
                    onClick={() => setQrCode('')}
                    className="inline-flex items-center px-6 py-3 bg-gray-600 text-white font-bold rounded-xl hover:bg-gray-700 transition-all"
                    >
                    Hide
                    </button>
                ...
                <p className="text-sm text-gray-500 mt-4">
                    ✓ QR Code ready. Show it to staff for access verification.
                </p>
                </div>
            )}
            </div>
